...
public class PostGameResults : MonoBehaviour 
{
	...
	public Image screen;
	GlobalManager globe;
	float danceFillAmount;
	float vocalFillAmount;
	float prFillAmount;
	float stressFillAmount;
	float danceStressFill;
	float vocalStressFill;
	float prStressFill;
	...
	void timedEvents()
	{
		...
		if (timer > showFinalScores && !showFinalScoresStart) 
		{
			...
			for (int i = 0; i < totalScores.Length; i++) 
			{
				...
				if (i == 0) {
					totalScores [i].GetComponent<Text> ().text = globe.effectiveDance.ToString();
				} else if (i == 1) {
					totalScores [i].GetComponent<Text> ().text = globe.effectiveVocal.ToString();
				} else {
					totalScores [i].GetComponent<Text> ().text = globe.effectivePR.ToString();
				}
			}
		}
	}
